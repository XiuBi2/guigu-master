    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <!-- 小图列表 -->
          <MyImageList :skuImageList="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in skuSaleAttrValueList" :key="spuSaleAttr.id">
                <dt class="title">{{ spuSaleAttr.saleAttrName }}</dt>
                <dd changepirce="0"  v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id"
                :class="{ active: spuSaleAttrValue.isChecked == 1 }"  @click="changeActive(spuSaleAttrValue, spuSaleAttr.spuSaleAttrValueList)">
...
<script>
import MyImageList from './ImageList/ImageList'
import MyZoom from './Zoom/Zoom'
...
export default {
  ...
  methods: {
    // 通过正则判断用户输入的数量
    changeSkuNum () {
      this.skuNum = /^[0-9]*$/.test(parseInt(this.skuNum)) ? String(parseInt(this.skuNum)).replace('.', '') : '1'
    },
    // 添加到购物车
    async addShopcar () {
